...
if os.path.exists("dqn/collisions.csv"):
    os.remove("dqn/collisions.csv")
if os.path.exists("dqn/max_wait_durations.csv"):
    os.remove("dqn/max_wait_durations.csv")
if os.path.exists("dqn/rewards.csv"):
    os.remove("dqn/rewards.csv")
...
state_size = len(env.get_hashable_state(state))  # Assuming the state is a 1D array
...
while True:
    ...
    if current_time - last_action_time >= action_interval:
        action_index = agent.act(hashable_state)
    ...
    agent.remember(hashable_state, action_index, reward, hashable_next_state, done)
    agent.replay()
